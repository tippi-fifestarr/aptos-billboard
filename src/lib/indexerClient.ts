...
// Test the public API endpoint now that permissions are configured
const INDEXER_API_URL = process.env.NEXT_PUBLIC_INDEXER_API_URL || 'https://api.testnet.aptoslabs.com/nocode/v1/api/cmbtx8djy013rs6015b87fsck/v1/graphql';
...
export class BillboardIndexerClient {
  ...
  private async executeQuery(query: string, variables: Record<string, unknown> = {}): Promise<{
    ...
    try {
      ...
      console.log('Using public API with Bearer token...');
